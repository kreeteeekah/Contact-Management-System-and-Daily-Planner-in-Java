...
import java.sql.*;
public class urel implements ActionListener 
{
    JFrame f1;
    JPanel p0,p1;
  ...
  public void actionPerformed(ActionEvent e)
  {
      if(e.getSource()==b1)
      {
           try
          {
              ...
              PreparedStatement stat=con.prepareStatement("update poject set crelation=? , where username=? and cname=?");
              stat.setString(1,a1);
              stat.executeUpdate();
              q1.showMessageDialog(null,"Relationship status has been updated.","Success",1);
          }
          catch(Exception ec)
          {
      ...
      if(e.getSource()==b2)
      {
          try
          {
              ...
              Class.forName("org.apache.derby.jdbc.ClientDriver");
              Connection con=DriverManager.getConnection("jdbc:derby://localhost:1527/sample","app","app");
              PreparedStatement stat=con.prepareStatement("delete from poject crelation=? , where username=? and cname=?");
              stat.setString(1,a1);
              stat.executeUpdate();
              q1.showMessageDialog(null,"Information deleted.","Success",1);
          }
          catch(Exception ec)
          {
              System.out.println(ec);
